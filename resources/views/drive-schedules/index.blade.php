    ...
    <div class="card" id="scheduleList">
        <div class="card-body">
            <div class="grid grid-cols-1 gap-5 mb-5 xl:grid-cols-2">
                <div>
                    <div class="relative xl:w-3/6">
                        <form action="{{ route('drive-schedules.index') }}" method="GET">
            ...
            <div class="overflow-x-auto">
                ...
                @endif
                <form action="{{ route('drive-schedules.bulkDelete') }}" method="POST" id="bulkDeleteForm">
                    @csrf
                    @method('DELETE')
                        ...
                        <tbody>
                            ...
                            @forelse($schedules as $schedule)
                                <tr>
                                    ...
                                    <td class="px-3.5 py-2.5 border-y border-slate-200">{{ $schedule->driver_id  ? $schedule->driver->name : '-' }}</td>
                                    ...
                                    <td class="px-3.5 py-2.5 border-y border-slate-200">{{ \Carbon\Carbon::parse($schedule->kedatangan)->format('Y-m-d H:i:s') }}</td>
                                    <td class="px-3.5 py-2.5 border-y border-slate-200">Rp. {{ $schedule->honor ? $schedule->honor : '-'}}</td>
                                    <td class="px-3.5 py-2.5 border-y border-slate-200">Rp. {{ $schedule->denda ? $schedule->denda : '-'}}</td>
                                    ...
                                    <td class="px-3.5 py-2.5 border-y border-slate-200">
                                        <div class="flex gap-2">
                                            ...
                                            <form action="{{ route('drive-schedules.destroy', $schedule->id) }}" method="POST" class="delete-form">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button" class="text-white bg-red-500 border-red-500 btn hover:text-white hover:bg-red-600 hover:border-red-600 focus:text-white focus:bg-red-600 focus:border-red-600 focus:ring focus:ring-red-100 active:text-white active:bg-red-600 active:border-red-600 active:ring active:ring-red-100 btn-delete">
    ...
    <div id="showModal" modal-center
        ...
        <div class="w-screen md:w-[30rem] bg-white shadow rounded-md">
            ...
            <div class="max-h-[calc(theme('height.screen')_-_180px)] overflow-y-auto p-4">
                <form action="{{ route('drive-schedules.update', 'schedule_id') }}" method="POST" id="showForm">
                    @csrf
                    @method('PUT')
                    <div class="mb-3">
