...
@section('content')
    ...
    <div class="card" id="scheduleList">
        <div class="card-body">
            <div class="grid grid-cols-1 gap-5 mb-5 xl:grid-cols-2">
                <div>
                    <div class="relative xl:w-3/6">
                        <form action="{{ route('service-schedules.index') }}" method="GET">
            ...
            <div class="overflow-x-auto">
                ...
                @endif
                <form action="{{ route('service-schedules.bulkDelete') }}" method="POST" id="bulkDeleteForm">
                    @csrf
                    @method('DELETE')
                        ...
                        <tbody>
                            ...
                            @forelse($schedules as $schedule)
                                <tr>
                                    ...
                                    <td class="px-3.5 py-2.5 border-y border-slate-200">{{ $schedule->vehicle->name . ' - ' . $schedule->vehicle->license_plate }}</td>
                                    <td class="px-3.5 py-2.5 border-y border-slate-200">{{ \Carbon\Carbon::parse($schedule->service_date)->format('Y-m-d H:i:s') }}</td>
                                    <td class="px-3.5 py-2.5 border-y border-slate-200">{{ $schedule->keterangan ? $schedule->keterangan : '-' }}</td>
                                    ...
                                    <td class="px-3.5 py-2.5 border-y border-slate-200">
                                        <div class="flex gap-2">
                                            <a href="javascript:void(0);" class="text-white bg-yellow-500 border-yellow-500 btn hover:text-white hover:bg-yellow-600 hover:border-yellow-600 focus:text-white focus:bg-yellow-600 focus:border-yellow-600 focus:ring focus:ring-yellow-100 active:text-white active:bg-yellow-600 active:border-yellow-600 active:ring active:ring-yellow-100"
                                                ...
                                                onclick="openEditModal({{ $schedule->id }}, '{{ $schedule->service_date }}', '{{ $schedule->vehicle_id }}','{{ $schedule->keterangan }}', '{{ $schedule->status }}')">
                                                <i class="ri-edit-line"></i> Edit
                                            </a>

                                            <form action="{{ route('service-schedules.destroy', $schedule->id) }}" method="POST" class="delete-form">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button" class="text-white bg-red-500 border-red-500 btn hover:text-white hover:bg-red-600 hover:border-red-600 focus:text-white focus:bg-red-600 focus:border-red-600 focus:ring focus:ring-red-100 active:text-white active:bg-red-600 active:border-red-600 active:ring active:ring-red-100 btn-delete">
            ...
            <div id="showModal" modal-center
                ...
                <div class="w-screen md:w-[30rem] bg-white shadow rounded-md">
                    <div class="flex items-center justify-between p-4 border-b border-slate-200">
                        <h5 class="text-16" id="exampleModalLabel">Add Service Schedule</h5>
                    ...
                    <div class="max-h-[calc(theme('height.screen')_-_180px)] overflow-y-auto p-4">
                        <form class="tablelist-form" action="{{ route('service-schedules.store') }}" method="POST">
                            @csrf
                            <div class="mb-3">
                                <label for="vehicle_id" class="inline-block mb-2 text-base font-medium">Vehicle</label>
                                <select name="vehicle_id" id="vehicle_id" class="form-input border-slate-200 focus:outline-none focus:border-custom-500">
                            ...
                            <div class="mb-3">
                                <label for="service_date" class="inline-block mb-2 text-base font-medium">Tanggal</label>
                                <input type="date" name="service_date" id="service_date" class="form-input border-slate-200 focus:outline-none focus:border-custom-500">
                            </div>
                            <div class="mb-3">
                                <label for="keterangan" class="inline-block mb-2 text-base font-medium">Keterangan</label>
                                <textarea name="keterangan" id="keterangan" class="form-input border-slate-200 focus:outline-none focus:border-custom-500"></textarea>
                            </div>
    ...
    <div id="showModal" modal-center
        ...
        <div class="w-screen md:w-[30rem] bg-white shadow rounded-md">
            <div class="flex items-center justify-between p-4 border-b border-slate-200">
                <h5 class="text-16" id="exampleModalLabel">Edit Service Schedule</h5>
                <button data-modal-close="showModal" type="button"
                    class="transition-all duration-200 ease-linear "><i data-lucide="x"
                        class="size-5"></i></button>
            </div>
            <div class="max-h-[calc(theme('height.screen')_-_180px)] overflow-y-auto p-4">
                <form action="{{ route('service-schedules.update', 'schedule_id') }}" method="POST" id="showForm">
                    @csrf
                    @method('PUT')
                    <div class="mb-3">
                        <label for="vehicle_id" class="inline-block mb-2 text-base font-medium">Vehicle</label>
                        <select name="vehicle_id" id="vehicle_id" class="form-input border-slate-200 focus:outline-none focus:border-custom-500">
                            <option value="">Select Vehicle</option>
                            @foreach($vehicles as $vehicle)
                                <option value="{{ $vehicle->id }}"{{ old('vehicle_id',$vehicle->id) }}>{{ $vehicle->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-3">
...
@push('scripts')
    ...
    <script>
        ...
        function openEditModal(scheduleId, service_date, driverId,keterangan, status) {
            // Set the form action to the correct route with the schedule ID
            const formAction = `/service-schedules/${scheduleId}`;
            document.getElementById('showForm').action = formAction;
    
            // Set the service_date field
            document.getElementById('service_date').value = service_date;
    
            // Set the selected driver
            document.getElementById('vehicle_id').value = driverId;
