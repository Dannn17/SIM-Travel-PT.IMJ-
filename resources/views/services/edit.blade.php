...
@section('title', __('Edit Service Entry'))
...
<x-page-title title="Edit Service Entry" pagetitle="Service Data" />
        ...
        <form action="{{ route('services.update', $service->id) }}" method="POST">
            ...
            <div class="grid grid-cols-1 gap-4 mb-5">
                <div>
                    ...
                    <select id="vehicle_id" name="vehicle_id" class="form-input w-full border-slate-200">
            ...
            <div class="grid grid-cols-2 gap-4">
                @if ($service->details->booking_id == null)
                <div>
                    <label for="tanggal_service" class="block font-medium">Service Date <span class="text-red-500">*</span></label>
                    <input type="date" id="tanggal_service" name="tanggal_service" value="{{ old('tanggal_service', $service->details->tanggal_service) }}" required class="form-input w-full border-slate-200">
                ...
                <div>
                    ...
                    <textarea id="keterangan" name="keterangan" class="form-input w-full border-slate-200">{{ old('keterangan', $service->details->keterangan) }}</textarea>
                    @error('keterangan') <p class="text-red-500 text-sm">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label for="quantity" class="block font-medium">Quantity <span class="text-red-500">*</span></label>
                    <input type="number" step="0.01" id="quantity" name="quantity" value="{{ old('quantity', $service->details->quantity) }}" class="form-input w-full border-slate-200">
                ...
                <div>
                    ...
                    <input type="number" step="0.01" id="jumlah" name="jumlah" value="{{ old('jumlah', $service->details->jumlah) }}" required class="form-input w-full border-slate-200">
            ...
            <div class="flex justify-end gap-2 mt-6">
                <button type="submit" class="text-white btn bg-custom-500 border-custom-500 hover:bg-custom-600">
                    ...
                    <span class="align-middle">Save Change</span>
                </button>
            </div>
