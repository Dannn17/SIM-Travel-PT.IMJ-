...
@section('content')
<!-- page title -->
<x-page-title title="Service List" pagetitle="Services" />

<div class="card" id="serviceList">
    <div class="card-body">
        <div class="flex flex-wrap items-center justify-between gap-4 mb-5">
            ...
            <form action="{{ route('services.index') }}" method="GET" class="flex items-center gap-2">
                <div class="flex border border-slate-200 rounded-md overflow-hidden">
                    <input type="text" name="search"
                        class="flex-1 form-input border-none focus:outline-none focus:ring-0 disabled:bg-slate-100 placeholder:text-slate-400 px-3 py-2"
                        placeholder="Search for services..." value="{{ $search }}" autocomplete="off">
            ...
            <form action="{{ route('services.index') }}" method="GET" class="flex items-center gap-2">
                <div class="flex border border-slate-200 rounded-md overflow-hidden">
                    ...
                    <input type="date" name="start_date" value="{{ request('start_date') }}" 
                           class="form-input border-none focus:outline-none focus:ring-0 disabled:bg-slate-100 px-3 py-2" 
            ...
            <div class="flex items-center gap-2">
                <button type="button" data-bs-toggle="modal" data-bs-target="#showModal" data-modal-target="showModal"
                        ...
                        <i class="align-bottom ri-printer-line me-1"></i> Create Report
                    </button>
                <a href="{{ route('services.create') }}"
                    class="text-white btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100 add-btn px-4 py-2 rounded-md">
                    <i class="align-bottom ri-add-line me-1"></i> Add Service
                </a>
        ...
        <div class="overflow-x-auto">
            ...
            @endif
            <form action="{{ route('services.bulkDelete') }}" method="POST" id="bulkDeleteForm">
                @csrf
                @method('DELETE')
                <table class="w-full whitespace-nowrap" id="serviceTable">
                    <thead class="bg-slate-100">
                        <tr>
                            ...
                            <th class="px-3.5 py-2.5 font-semibold border-b border-slate-200 ltr:text-left rtl:text-right">Description</th>
                            <th class="px-3.5 py-2.5 font-semibold border-b border-slate-200 ltr:text-left rtl:text-right">Total Charge</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="px-3.5 py-2.5 border-y border-slate-200">{{ $item->vehicle_id ? $item->vehicle->name . ' / ' . $item->vehicle->license_plate : '-' }}</td>
                            <td class="px-3.5 py-2.5 border-y border-slate-200">{{ $item->details->keterangan ? $item->details->keterangan : '-' }}</td>
                            <td class="px-3.5 py-2.5 border-y border-slate-200">{{ ($item->details->booking_id ? '' : '- ') . 'Rp. ' . $item->total }}</td>
                            <td class="px-3.5 py-2.5 border-y border-slate-200">
                                <form action="{{ route('services.destroy', $item->id) }}" method="POST" class="delete-form">
                                    ...
                                    <div class="flex gap-2">
                                        ...
                                        <button type="submit" id="sa-params" class="text-xs text-white bg-red-500 border-red-500 btn hover:text-white hover:bg-red-600 hover:border-red-600 focus:text-white focus:bg-red-600 focus:border-red-600 focus:ring focus:ring-red-100 active:text-white active:bg-red-600 active:border-red-600 active:ring active:ring-red-100 btn-delete">
