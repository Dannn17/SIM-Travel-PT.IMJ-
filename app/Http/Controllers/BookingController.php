...
public function getAvailableVehicles(Request $request)
{
    ...
    // 1. Ambil ID kendaraan yang sedang dibooking pada rentang waktu yang diberikan
    ...
    $scheduledVehicleIds = ServiceSchedule::whereBetween('service_date', [$startDate, $endDate])
        ->whereNotIn('status', ['done', 'cancel']) 
        ->pluck('vehicle_id');


    $unavailableVehicleIds = $bookedVehicleIds->merge($scheduledVehicleIds)->unique();
...
public function getVehicleStatus(Request $request)
{
    ...
    $vehicleStatus = $vehicles->map(function ($vehicle) use ($scheduledVehicleIds) {
       
        $isBooked = $vehicle->bookings->isNotEmpty();
       
        $isScheduledForService = $scheduledVehicleIds->contains($vehicle->id); 
        
      
        $isUnavailable = $isBooked || $isScheduledForService; 
        ...
        return [
            ...
            'is_scheduled_for_service' => $isScheduledForService, 
        ];
    });
    ...
    public function updateDeposit(Request $request, $id)
    {
        ...
        if( $booking->deposit != 0)
        {
            ...
            ServiceDetail::create([
                ...
                'keterangan' => 'Deposit sewa ' . $booking->customer->name,
    ...
    public function markAsPaid($id)
    {
        ...
        if (in_array($booking->status, ['pending', 'deposit'])) {
            ...
            ServiceDetail::create([
                ...
                'booking_id' => $booking->id,
                'tanggal_service' => $tanggalService,
                'keterangan' => 'Pelunasan sewa ' . $booking->customer->name,
                'jumlah' => $booking->balance,
                'debet' => $booking->balance,
                'kredit' => 0,
            ]);
            $booking->update(['balance' => 0]);
            return redirect()->route('bookings.index')->with('success', 'Booking status updated to paid.');
        }

        return redirect()->route('bookings.index')->with('error', 'Cannot update booking status.');
    }
